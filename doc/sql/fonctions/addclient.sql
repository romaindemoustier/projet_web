...
'
    ...
    declare id integer;
    begin
        insert into client(nom, prenom, adresse, ville, cp, pays, numdetel) values (f_nom, 
f_pr, f_a, f_v, f_c, f_pa, f_tel);
        select into id idclient from client where nom=f_nom and prenom=f_pr and adresse=f_a 
and ville=f_v and cp=f_c and pays=f_pa and numdetel=f_tel;
	if not found then
	    id=0;
	end if;
	return id;
